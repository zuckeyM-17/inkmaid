# 📝 Inkmaid

**手書きとAIで直感的に図解するフルスタックプラットフォーム**

ブラウザ上での「手書き」という直感的な入力と、AIエージェントによる「論理的な構造操作」を融合させ、専門知識がなくても高度なMermaid図（フローチャート、ER図、シーケンス図等）を即座に作成・編集・管理できる環境を提供します。

---

## 🚀 クイックスタート

### 前提条件

以下がインストールされていることを確認してください：

- **Node.js** >= 20.0.0
- **pnpm** >= 9.15.1
- **Docker** & **Docker Compose**

```bash
# pnpmがない場合はインストール
corepack enable && corepack prepare pnpm@9.15.1 --activate
```

### セットアップ

```bash
# 1. リポジトリをクローン
git clone <repository-url>
cd inkmaid

# 2. 依存関係をインストール
pnpm install

# 3. 環境変数を設定
cp .env.example .env

# 4. 開発環境を起動（DB + Next.js）
pnpm dev:all
```

ブラウザで http://localhost:3000 を開くと、アプリケーションが表示されます。

---

## 🛠️ 技術スタック

| カテゴリ | 技術 | 説明 |
|---------|------|------|
| **Framework** | Next.js 15 (App Router) | フルスタックReactフレームワーク |
| **Language** | TypeScript 5 | 型安全な開発 |
| **Styling** | Tailwind CSS 4 | ユーティリティファーストCSS |
| **API** | tRPC | 型安全なクライアント/サーバー通信 |
| **Database** | PostgreSQL 16 | リレーショナルデータベース |
| **ORM** | Drizzle ORM | TypeScriptフレンドリーなORM |
| **AI** | Vercel AI SDK | LLMストリーミング・Tool Calling |
| **Linter** | Biome | 高速なLinter/Formatter (Rust製) |
| **Testing** | Vitest | 高速なユニットテスト |
| **Package Manager** | pnpm | 効率的なパッケージ管理 |
| **Container** | Docker Compose | 開発/本番環境の統一 |

---

## 📁 ディレクトリ構成

```
.
├── apps/
│   └── web/                      # Next.js アプリケーション
│       ├── src/
│       │   ├── app/              # App Router ページ
│       │   │   ├── api/trpc/     # tRPC APIエンドポイント
│       │   │   ├── layout.tsx    # ルートレイアウト
│       │   │   └── page.tsx      # トップページ
│       │   ├── components/       # UIコンポーネント（今後追加）
│       │   ├── lib/
│       │   │   └── trpc/         # tRPCクライアント設定
│       │   └── server/
│       │       ├── db/           # Drizzle スキーマ・クライアント
│       │       └── trpc/         # tRPC ルーター定義
│       ├── drizzle.config.ts     # Drizzle設定
│       ├── Dockerfile            # 本番用Dockerfile
│       └── package.json
├── doc/                          # プロジェクトドキュメント
│   ├── develop.md                # 設計ドキュメント
│   └── 2025-12-21.md             # 開発ログ
├── docker-compose.yml            # 開発用（PostgreSQLのみ）
├── docker-compose.prod.yml       # 本番用（PostgreSQL + Web）
├── biome.json                    # Biome設定
├── pnpm-workspace.yaml           # pnpmワークスペース設定
└── package.json                  # ルートパッケージ設定
```

---

## 📋 コマンド一覧

### 開発

| コマンド | 説明 |
|---------|------|
| `pnpm dev` | Next.js開発サーバーを起動 |
| `pnpm dev:all` | DB起動 → Next.js起動（推奨） |
| `pnpm build` | プロダクションビルド |
| `pnpm start` | ビルド済みアプリを起動 |

### データベース

| コマンド | 説明 |
|---------|------|
| `pnpm db:up` | PostgreSQLコンテナを起動 |
| `pnpm db:down` | PostgreSQLコンテナを停止 |
| `pnpm db:logs` | PostgreSQLのログを表示 |
| `pnpm db:reset` | データを削除して再起動 |
| `pnpm db:push` | スキーマをDBに反映 |
| `pnpm db:generate` | マイグレーションファイル生成 |
| `pnpm db:migrate` | マイグレーション実行 |
| `pnpm db:studio` | Drizzle Studio（DB GUI）を起動 |

### コード品質

| コマンド | 説明 |
|---------|------|
| `pnpm lint` | Biomeでコードをチェック |
| `pnpm lint:fix` | 問題を自動修正 |
| `pnpm format` | コードをフォーマット |
| `pnpm test` | Vitestでテスト実行 |

### 本番環境

| コマンド | 説明 |
|---------|------|
| `pnpm prod:build` | 本番用Dockerイメージをビルド |
| `pnpm prod:up` | 本番環境を起動（DB + Web） |
| `pnpm prod:down` | 本番環境を停止 |
| `pnpm prod:logs` | 本番環境のログを表示 |

---

## 🔧 開発ワークフロー

### 1. 日常的な開発

```bash
# 開発環境を起動
pnpm dev:all

# http://localhost:3000 で開発
# ファイルを編集するとホットリロードされます
```

### 2. データベーススキーマの変更

```bash
# 1. apps/web/src/server/db/schema.ts を編集

# 2. スキーマをDBに反映
pnpm db:push

# または、マイグレーションファイルを生成
pnpm db:generate
pnpm db:migrate
```

### 3. コミット前のチェック

```bash
# Lintとフォーマット
pnpm lint:fix

# テスト実行
pnpm test
```

### 4. 本番ビルドの確認

```bash
# 本番環境をローカルで起動
pnpm prod:build
pnpm prod:up

# http://localhost:3000 で確認
```

---

## 🗄️ データベース構成

### テーブル一覧

| テーブル | 説明 |
|---------|------|
| `projects` | プロジェクト（図）の基本情報 |
| `diagram_versions` | Mermaidコードのバージョン履歴 |
| `chat_sessions` | AIとの対話セッション |
| `chat_messages` | チャットメッセージ履歴 |
| `handwriting_strokes` | 手書きストロークデータ |

### 接続情報（開発環境）

| 項目 | 値 |
|-----|-----|
| Host | localhost |
| Port | 5432 |
| User | inkmaid |
| Password | inkmaid_password |
| Database | inkmaid |

---

## 🔌 API エンドポイント

tRPCを使用した型安全なAPIを提供しています。

### diagram ルーター

| エンドポイント | メソッド | 説明 |
|---------------|---------|------|
| `diagram.listProjects` | Query | プロジェクト一覧を取得 |
| `diagram.createProject` | Mutation | 新規プロジェクトを作成 |
| `diagram.getProject` | Query | プロジェクト詳細を取得 |
| `diagram.getVersionHistory` | Query | バージョン履歴を取得 |
| `diagram.saveVersion` | Mutation | 新しいバージョンを保存 |

### クライアントでの使用例

```tsx
import { trpc } from "@/lib/trpc/client";

// プロジェクト一覧を取得
const { data: projects } = trpc.diagram.listProjects.useQuery();

// プロジェクトを作成
const createProject = trpc.diagram.createProject.useMutation();
createProject.mutate({ name: "My Diagram" });
```

---

## ❓ トラブルシューティング

### ポート3000が使用中

```bash
# 使用中のプロセスを確認
lsof -i :3000

# プロセスを終了
kill -9 <PID>
```

### データベース接続エラー

```bash
# PostgreSQLが起動しているか確認
pnpm db:logs

# コンテナを再起動
pnpm db:restart
```

### node_modulesの問題

```bash
# キャッシュをクリアして再インストール
rm -rf node_modules apps/web/node_modules
pnpm install
```

### Drizzleのスキーマ同期

```bash
# スキーマを強制的に再適用
DATABASE_URL="postgresql://inkmaid:inkmaid_password@localhost:5432/inkmaid" pnpm db:push
```

---

## 📚 ドキュメント

- [設計ドキュメント](./doc/develop.md) - プロジェクトの詳細設計
- [開発ログ](./doc/logs/) - 日々の開発記録（`yyyymmdd.md` 形式）

---

## 🗺️ ロードマップ

### Phase 1: 基盤構築 ✅

- [x] pnpm ワークスペースのセットアップ
- [x] Docker + PostgreSQL 環境
- [x] Drizzle ORM スキーマ定義
- [x] tRPC API 基盤

### Phase 2: コア機能の実装

- [ ] 手書きCanvas（Konva.js）
- [ ] Mermaid.js による図の描画
- [ ] Vercel AI SDK による AIチャットボット

### Phase 3: 高度な編集と最適化

- [ ] 手書きジェスチャー認識
- [ ] バージョン履歴のタイムトラベルUI
- [ ] テストの充実

---

## 🤝 コントリビューション

1. 機能ブランチを作成: `git checkout -b feature/amazing-feature`
2. 変更をコミット: `git commit -m 'Add amazing feature'`
3. ブランチをプッシュ: `git push origin feature/amazing-feature`
4. プルリクエストを作成

---

## 📝 ライセンス

このプロジェクトは [MIT License](./LICENSE) の下で公開されています。

